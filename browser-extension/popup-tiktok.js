    ...
    const results = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        for (const { video, score } of videos.slice(0, 3)) {
          ...
          // Ask the already installed identity resolver first. Its MAIN-world
          // probe can read TikTok framework state that a popup injection cannot.
          const source = String(video.currentSrc || video.src || '');
          try {
            const resolved = globalThis.ApocalipseTikTokIdentity?.resolve?.(video);
