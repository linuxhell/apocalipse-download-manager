(() => {
  ...
  const root = document.createElement('section'); root.className = 'diag-controls';
  root.innerHTML = '<div><button type="button" data-action="start"></button><button type="button" data-action="mark"></button><button type="button" data-action="stop"></button></div><small role="status"></small><p></p>';
  document.querySelector('nav').before(root);
