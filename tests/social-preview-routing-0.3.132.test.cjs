...
test('0.3.159 manifest version is exact', () => {
  const manifest = JSON.parse(src('manifest.json'));
  assert.equal(manifest.version, '0.3.159');
  assert.match(manifest.version_name, /Facebook Home recycled-player overlay fix/);
});
