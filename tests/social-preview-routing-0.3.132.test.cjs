...
test('0.3.142 manifest version is exact', () => {
  const manifest = JSON.parse(src('manifest.json'));
  assert.equal(manifest.version, '0.3.142');
  assert.match(manifest.version_name, /Bind TikTok parent identity and exact Facebook ad label/);
});
