...
test('0.3.143 manifest version is exact', () => {
  const manifest = JSON.parse(src('manifest.json'));
  assert.equal(manifest.version, '0.3.143');
  assert.match(manifest.version_name, /Preserve TikTok feed identity and exact Facebook post scope/);
});
