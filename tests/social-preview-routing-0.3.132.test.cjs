...
test('0.3.141 manifest version is exact', () => {
  const manifest = JSON.parse(src('manifest.json'));
  assert.equal(manifest.version, '0.3.141');
  assert.match(manifest.version_name, /Restore late video Download and recording tracks/);
});
