...
fn request(url: &str) -> MediaPreviewRequest {
    MediaPreviewRequest {
        url: url.into(),
        user_agent: Some("SyntheticBrowser/1.0 Test".into()),
...
fn running(request: MediaPreviewRequest) -> Running {
    let relay = Relay::bind(request, Client::builder().no_proxy()).unwrap();
    let target = relay.target.clone();
    let (stop, receiver) = oneshot::channel();
    let logs = Arc::new(Mutex::new(Vec::new()));
    let sink = logs.clone();
    let reporter: Reporter =
        Arc::new(move |event, detail| sink.lock().unwrap().push(format!("{event} {detail}")));
...
fn only_direct_tiktok_media_uses_the_new_transport() {
    let mut capture =
        request("https://v16-webapp-prime.tiktok.com/video/tos/synthetic/?mime_type=video_mp4");
    assert!(is_candidate(&capture));
    capture.url = "https://v16.tiktokcdn.com/clip.mp4?signature=synthetic".into();
    assert!(is_candidate(&capture));
    capture.url = "https://unrelated.example/clip.mp4".into();
    assert!(!is_candidate(&capture));
    capture.url = "https://tiktok.com.unrelated.example/clip.mp4".into();
    assert!(!is_candidate(&capture));
    capture.url = "https://v16.tiktok.com/master.m3u8".into();
    assert!(!is_candidate(&capture));
    capture.url = "https://v16.tiktok.com/opaque".into();
    capture.content_type = Some("application/dash+xml".into());
    assert!(!is_candidate(&capture));
    capture.url = "https://www.tiktok.com/@creator/video/123".into();
    capture.content_type = None;
    assert!(!is_candidate(&capture));
    capture.url = "magnet:?xt=urn:btih:synthetic".into();
    assert!(!is_candidate(&capture));
}
#[tokio::test]
async fn signed_url_headers_and_seek_ranges_are_preserved_exactly() {
    ...
    .await;
    let path =
        "/video/tos/synthetic/?a=1988&&mime_type=video_mp4&signature=a%2Bb%3D&space=%20&plus=a+b";
    ...
    assert_eq!(answer.headers()["content-range"], "bytes 2-5/8");
    assert_eq!(answer.headers()["content-length"], "4");
    assert_eq!(answer.bytes().await.unwrap().as_ref(), b"cdef");
    let got = origin.received.lock().unwrap()[0].clone();
    assert!(
        got.starts_with(&format!("GET {path} HTTP/1.1\r\n")),
        "{got}"
    );
    ...
    assert!(!logs.contains("synthetic-secret") && !logs.contains("signature="));
    stop(server).await;
}
#[tokio::test]
async fn redirects_do_not_receive_cookies_or_expose_location_to_player() {
    let _serial = SERIAL.lock().await;
    let destination = fixture(vec![response(
        "200 OK",
    ...
    let origin = fixture(vec![response(
        "302 Found",
        &format!("Location: {}/next?signature=secret\r\n", destination.url),
        "",
    )])
    .await;
    let server = running(request(&format!("{}/original", origin.url)));
    let answer = client().get(&server.target).send().await.unwrap();
    ...
    assert!(origin.received.lock().unwrap()[0].contains("cookie:"));
    assert!(!destination.received.lock().unwrap()[0].contains("cookie:"));
    stop(server).await;
}
#[tokio::test]
async fn untrusted_local_requests_never_contact_the_source() {
    ...
    for builder in [
        client().get(format!("{}-wrong", server.target)),
        client()
            .get(&server.target)
    ...
    assert!(origin.received.lock().unwrap().is_empty());
    stop(server).await;
}
#[tokio::test]
async fn upstream_failures_are_reported_without_private_error_bodies() {
    ...
    let origin = fixture(vec![response(
        ...
        "private URL and account info",
    )])
    .await;
    let server = running(request(&origin.url));
    let answer = client().get(&server.target).send().await.unwrap();
    assert_eq!(answer.status(), StatusCode::BAD_GATEWAY);
    assert!(answer.bytes().await.unwrap().is_empty());
    let log = server.logs.lock().unwrap().join("\n");
    assert!(log.contains("preview_upstream_http_403"));
    assert!(!log.contains("account info") && !log.contains("synthetic-secret"));
    stop(server).await;
}
#[tokio::test]
async fn head_fallback_preserves_auth_and_invalid_ranges_have_no_body() {
    let _serial = SERIAL.lock().await;
    let origin = fixture(vec![
    ...
    .await;
    let server = running(request(&origin.url));
    let head = client().head(&server.target).send().await.unwrap();
    ...
    assert_eq!(answer.headers()["content-range"], "bytes */5");
    assert_eq!(answer.headers()["content-length"], "0");
    assert!(answer.bytes().await.unwrap().is_empty());
    let requests = origin.received.lock().unwrap().clone();
    assert!(requests[0].starts_with("HEAD "));
    assert!(
        requests[1].starts_with("GET ") && requests[1].contains("cookie: session=synthetic-secret")
    );
    stop(server).await;
}
#[tokio::test]
async fn overlapping_previews_keep_separate_sources_credentials_and_bytes() {
    let _serial = SERIAL.lock().await;
    let a = fixture(vec![response(
        "200 OK",
        "Content-Type: video/mp4\r\n",
        "video-a",
    )])
    .await;
    let b = fixture(vec![response(
        "200 OK",
        "Content-Type: video/webm\r\n",
        "video-b",
    )])
    .await;
    let mut second = request(&b.url);
    second.cookie_header = Some("session=other".into());
    let sa = running(request(&a.url));
    let sb = running(second);
    ...
    let (ra, rb) = tokio::join!(fetch(&sa.target), fetch(&sb.target));
    assert_eq!(ra, "video-a");
    assert_eq!(rb, "video-b");
    assert!(!a.received.lock().unwrap()[0].contains("session=other"));
    assert!(!b.received.lock().unwrap()[0].contains("synthetic-secret"));
    stop(sa).await;
    stop(sb).await;
}
#[tokio::test]
async fn unused_previews_expire_and_close_the_port() {
    ...
    let relay = Relay::bind(
        request("https://media.example/video"),
...
fn rejects_query_only_urls_and_header_injection() {
    for url in [
        "?a=1988",
        "file:///etc/passwd",
        "https://name:secret@media.example/v",
        "https://media.example/v\r\n--flag",
    ] {
        assert!(validate(&request(url)).is_err());
    }
    let mut bad = request("https://media.example/v");
    bad.cookie_header = Some("sid=one\r\nHost: evil".into());
    assert!(validate(&bad).is_err());
    bad.cookie_header = None;
    bad.referer = Some("javascript:alert(1)".into());
    assert!(validate(&bad).is_err());
}
#[test]
fn portable_player_path_and_local_url_are_separate_arguments() {
    let root = std::env::temp_dir().join(format!("adm vlc test {}", uuid::Uuid::new_v4()));
    let player = root.join("App/vlc/vlc.exe");
    std::fs::create_dir_all(player.parent().unwrap()).unwrap();
    std::fs::write(&player, b"test fixture").unwrap();
    assert_eq!(effective_player(&root.join("VLCPortable.exe")), player);
    let target = "http://127.0.0.1:1234/random/media";
    let command = player_command(&player, target);
    assert_eq!(command.get_program(), player.as_os_str());
    assert_eq!(command.get_current_dir(), player.parent());
    let args = command
        .get_args()
    ...
    if cfg!(windows) {
        assert!(args.iter().any(|arg| arg == "--no-one-instance"));
    }
