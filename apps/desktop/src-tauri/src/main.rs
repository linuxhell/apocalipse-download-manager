...
fn vault_store_verified(account: &str, secret: &str) -> Result<(), String> {
    ...
    let recovered = entry.get_password().map_err(|error| error.to_string())?;
    if recovered != secret {
...
async fn download_with_mirrors(
    ...
    engine
        .download_verified_sources(request, sources, events)
        .await
}
...
mod tests {
    ...
    #[test]
    fn serialized_settings_never_contain_runtime_secrets() {
        let mut settings = UserSettings::default();
        settings.bridge_token = "bridge-secret".into();
        settings.link_password = "link-secret".into();
        settings.proxy_password = Some("proxy-secret".into());
        settings.website_credentials.push(WebsiteCredential {
            host: "example.com".into(),
            username: "alice".into(),
            password: "website-secret".into(),
        });
