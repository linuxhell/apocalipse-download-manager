(function (root, factory) {
  ...
  const copy = {
    en: {
      ...
      toolsHelp: "Tools manages FFmpeg, FFprobe, yt-dlp, aria2 (including torrent and magnet downloads), N_m3u8DL-RE, QuickJS and your external media player.",
    ...
    "pt-BR": {
      ...
      toolsHelp: "Ferramentas administra FFmpeg, FFprobe, yt-dlp, aria2 (incluindo downloads de torrent e magnet), N_m3u8DL-RE, QuickJS e o seu player externo.",
    ...
    "zh-CN": {
      ...
      toolsHelp: "“工具”用于管理 FFmpeg、FFprobe、yt-dlp、aria2（含种子和磁力链接下载）、N_m3u8DL-RE、QuickJS 和你的外部播放器。",
  ...
  function debuggerDomainDiagnosis(question, context, locale) {
    const q = normalizeQuestion(question);
    const domains = [
      { name: "aria2", test: /aria2/, event: /^(?:aria2\.|http\.engine_selected)/ },
      { name: "torrent", test: /torrent|magnet/, event: /^(?:aria2\.|torrent\.)/ },
  ...
  function diagnose(question, context, locale) {
    ...
    if (/(ffmpeg|ffprobe|yt-dlp|aria2|m3u8dl|quickjs|ferrament|tool|工具|软件工具)/.test(q)) return say(locale, "toolsHelp");
